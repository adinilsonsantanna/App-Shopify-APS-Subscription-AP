...
function dependencies(result: unknown = { data: { subscriptionContractPause: { contract: { id: valid.contractId, status: "PAUSED" }, userErrors: [] } } }, currentStatus = "ACTIVE") {
  const calls: Array<{ query: string; variables: Record<string, unknown> }> = [];
  return { calls, value: { apiKey: "secret", getAdmin: async () => ({ session: { shop: valid.shop }, admin: { graphql: async (query: string, options: { variables: Record<string, unknown> }) => { calls.push({ query, variables: options.variables }); return Response.json(query.includes("LifecycleContractStatus") ? { data: { subscriptionContract: { id: valid.contractId, status: currentStatus } } } : result); } } }) } };
}
...
test("executes pause, resume and cancel with actor", async () => {
  for (const [action, field, status] of [["pause", "subscriptionContractPause", "PAUSED"], ["resume", "subscriptionContractActivate", "ACTIVE"], ["cancel", "subscriptionContractCancel", "CANCELLED"]] as const) {
    const deps = dependencies(
      { data: { [field]: { contract: { id: valid.contractId, status }, userErrors: [] } } },
      action === "resume" ? "PAUSED" : "ACTIVE",
    );
    const response = await handleSubscriptionLifecycle(request({ ...valid, action, actor: "MERCHANT" }), deps.value);
    assert.equal(response.status, 200); assert.match(deps.calls[1].query, new RegExp(field)); assert.equal(deps.calls[1].variables.actor, "MERCHANT");
  }
});
...
test("already applied action is reconciled without repeating the mutation", async () => { const deps = dependencies(undefined, "PAUSED"); const response = await handleSubscriptionLifecycle(request(), deps.value); assert.equal(response.status, 200); assert.equal(deps.calls.length, 1); assert.equal((await response.json() as any).duplicate, true); });
